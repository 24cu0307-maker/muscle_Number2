...
public class ExcelLoader : MonoBehaviour
{
    ...
    public ExcelRankingLoader excelRankingLoader;
    ...
    private void Awake()
    {
        ...
        excelRankingLoader = new ExcelRankingLoader();

        //CSVファイルを読み込み
        excelPoseJudgeLoader.LoadCsv();
        excelPoseTimeFlowLoader.LoadCsv("PoseTimeFlow");
        excelRankingLoader.LoadCsv("ExcelRanking");
    }
...
public class ExcelPoseTimeFlowLoader
{
    ...
    public void LoadCsv(string _excelName)
    {
        ...
        for (int i = 1; i < lines.Length; i++)
        {
            ...
            //5列目は任意。空欄または旧CSVでは従来どおりランダム演出を使用する
